...
const { requiredMessage, minCountMessage, maxCountMessage, invalidEmail, onlyAlphabets } = CommonValidations
...
export const SignUpSchema: z.ZodType<SignupPayloadTypes> = z
  .object({
    firstName: z
      ...
      .min(SchemaConstants.firstName.minCount, minCountMessage(registerBuyerLabel.firstName, SchemaConstants.firstName.minCount))
      .max(SchemaConstants.firstName.maxCount, maxCountMessage(registerBuyerLabel.firstName, SchemaConstants.firstName.minCount)),
    lastName: z
      .string()
      .min(1, requiredMessage(registerBuyerLabel.lastName))
      .regex(ALPHABETS_REGEX, onlyAlphabets(registerBuyerLabel.lastName))
      .min(SchemaConstants.lastName.minCount, minCountMessage(registerBuyerLabel.lastName, SchemaConstants.lastName.minCount))
      .max(SchemaConstants.lastName.maxCount, maxCountMessage(registerBuyerLabel.lastName, SchemaConstants.lastName.minCount)),
    ...
    password: z
      ...
      .min(SchemaConstants.password.minCount, minCountMessage(registerBuyerLabel.password, SchemaConstants.password.minCount))
      .max(SchemaConstants.password.maxCount, maxCountMessage(registerBuyerLabel.password, SchemaConstants.password.maxCount)),
    confirmPassword: z.string().min(1, requiredMessage(registerBuyerLabel.confirmPassword)),
  })
  .required()
...
export const ForgetPasswordSchema: z.ZodType<ForgetPasswordPayloadTypes> = z.object({
  email: z.string().min(1, requiredMessage(registerBuyerLabel.email)).email({
    message: invalidEmail,
  }),
})

export const UpdateProfileSchema: z.ZodType<ForgetPasswordPayloadTypes> = z.object({
  first_name: z
    .string()
    .min(1, requiredMessage(registerBuyerLabel.firstName))
    .regex(ALPHABETS_REGEX, onlyAlphabets(registerBuyerLabel.firstName))
