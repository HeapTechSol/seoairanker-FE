...
export const firstChild = (elements: Element[]) =>
  elements.flatMap((item) => item.firstElementChild);

export const getElement = (element: Element, className: string) =>
  element ? element.querySelector(`.${className}`) : null;

export const getElements = (elements: Element, className: string) =>
  Array.from(elements.querySelectorAll(`.${className}`));
...
export const accessElements = (menuRef: React.RefObject<HTMLDivElement>) => {
  ...
  const subMenuWithActiveClass =
    subMenuItemWithActiveClass?.closest(".submenu");
