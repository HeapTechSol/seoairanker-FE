  ...
  return [
    {
      ...
      text: (
        <>
          Extra Sites from {strongTextGenerator(`$${extra_sites.toFixed(2)}`)}{" "}
          per {strongTextGenerator("1 site")}
        </>
      ),
    },
    {
      ...
      text: (
        <>
          Extra Keywords from{" "}
          {strongTextGenerator(`$${extra_keywords.toFixed(2)}`)} per{" "}
          {strongTextGenerator("100 keywords")}
        </>
      ),
    },
    {
      ...
      text: (
        <>
          Extra Pages from {strongTextGenerator(`$${extra_pages.toFixed(2)}`)}{" "}
          per {strongTextGenerator("250 pages")}
        </>
      ),
    },
  ] as addOnInfoTypes[];
  ...
  return [
    {
      ...
      text: (
        <>
          AI JSON-LD Schemas for{" "}
          {strongTextGenerator(`$${perSchema.toFixed(2)}`)} per schema
        </>
      ),
    },
    {
      ...
      text: (
        <>
          AI Meta Descriptions included, then{" "}
          {strongTextGenerator(`$${perDescription.toFixed(2)}`)} per description
        </>
      ),
    },
    {
      ...
      text: (
        <>
          AI Meta Titles included, then{" "}
          {strongTextGenerator(`$${perTitle.toFixed(2)}`)} per title
        </>
      ),
    },
    {
      ...
      text: (
        <>
          Keyword Rankings Updates included, then{" "}
          {strongTextGenerator(`$${perUpdate.toFixed(2)}`)} per update
        </>
      ),
    },
    {
      ...
      text: (
        <>
          Page Crawls included, then{" "}
          {strongTextGenerator(`$${perCrawl.toFixed(2)}`)} per crawl
        </>
      ),
    },
  ];
...
export const monthlyPlans: PlanTypes[] = [
  {
    ...
    detailsInfo: detailsInfoGenerator(
      500,
      500,
      2500,
      7500,
  ...
  },
  {
    ...
    type: "Enterprise",
    description:
      "For corporations, agencies, online retailers, tech hubs, etc.",
    isAPIAccess: true,
    crawlSchedule: "Daily",
    buttonText: "Get Started",
    buttonColor: "info",
...
export type PlansTitles = "basic" | "agency" | "enterprise";
