...
import { ConicalDataTypes } from '@/container/sites/sitesTypes'
import { sitesAPI } from '../../api/sitesAPI'
import { store, useAppSelector } from '@/api/store'

const ConicalTagPreview = ({ link_id: externalLinkId }: { link_id: string }) => {
  ...
  const handleAllRecommendations = async () => {
    if (siteId) {
      await handleUpdateRecommendations({
       ...
       getRecommendationByType({ page: 1, per_page: 10, type: 'cannonical_tags', link_id: externalLinkId })
    }
  }
  ...
  const accordionDescription = (item: ConicalDataTypes, index: number) => (
    <Flex vertical gap={4}>
      ...
      <Typography
        color="warning"
        text={item?.generated_canonical}
        contentEditable={item.id === editedId}
        onBlur={(e) => handleBlur(e, item.id, index, item?.generated_canonical, item.link_id)}
        ref={(el) => (editableRefs.current[index] = el)}
      />
    </Flex>
  )

  const optimizedTitlesList = (recommendationData?.data as ConicalDataTypes[])?.map((item, index) => ({
    url: item?.link_path,
  ...
  const handleLoadMore = () => {
    getRecommendationByType({ page: (recommendationData?.page || 0) + 1, per_page: 10, type: 'cannonical_tags', link_id: externalLinkId })
  }
      ...
      <ShimmerPlaceholder loading={recommendationDataLoading}>
        <Flex vertical gap={16}>
          <Flex align="start" padding="40px 40px 0px 40px">
            <Flex vertical gap={16}>
              <Typography type="h3" text="Add Canonical URL Tag" />
              <Typography text="A canonical link is an HTML tag that shows Google what the official URL is for that page and helps to prevent potential duplicate content issues. Approve the recommendation and pages to have canonical URLs added automatically to the following pages." />
            </Flex>
            <Button
              size="sm"
              variant="outlined"
...
export default ConicalTagPreview