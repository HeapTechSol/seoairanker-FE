...
const MetaKeywords = ({ link_id: externalLinkId }) => {
  ...
  const refreshRecommendations = async () => {
    ...
    await getRecommendationByType({ page: 1, per_page: 10, type: 'keywords', link_id: externalLinkId })
  }
  ...
  const accordionDescription = (item, index) => (
    <Flex vertical gap={4}>
      <Typography
        color="warning"
        text={'Top Keywords: ' + item?.keywords}
        contentEditable={item.id === editedId}
        onBlur={(e) => handleBlur(e, item.id, index, item?.keywords, item.link_id)}
  ...
  const handleLoadMore = () => {
    getRecommendationByType({ page: (recommendationData?.page || 0) + 1, per_page: 10, type: 'keywords', link_id: externalLinkId })
  }

  useEffect(() => {
    getRecommendationByType({ page: 1, per_page: 10, type: 'keywords', link_id: externalLinkId })
      ...
      <ShimmerPlaceholder loading={recommendationDataLoading}>
        <Flex vertical gap={16}>
          <Flex align="start" padding="40px 40px 0px 40px">
            <Flex vertical gap={16}>
              <Typography type="h3" text="Meta Keywords" />
              <Typography text="Meta Keywords to rank better in search" />
            </Flex>
            {/* <Button
              size="sm"
              variant="outlined"
